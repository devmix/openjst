 ...
 */
public final class ClientLoadTest {
    ...
    public static void main(final String[] arg) throws IOException, ClientNotConnectedException {
        ...
        for (int i = 0; i < CLIENTS; i++) {
            final TestClient client = new TestClient(HOST, ProtocolBasicConstants.DEFAULT_CLIENTS_PORT, executorService);
            client.start();
                ...
                public void run() {
                    try {
                        ...
                        for (int i = 0; i < ITERATION_MESSAGES; i++) {
                            client.sendRPC(RPCMessageFormat.BINARY, new byte[]{0});
                        }
...
//            }).start();
        ...
        public void start() {
            epsTotal = 0;
            iteration.set(0);
            if (!client.connect("account", String.valueOf(GEN_USER_ID.getAndIncrement()), SecretKeys.PLAIN.encode(""), null)) {
