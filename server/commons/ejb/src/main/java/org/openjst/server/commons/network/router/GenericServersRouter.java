 ...
 */
class GenericServersRouter<R extends Actor<?>, M extends Message<?>> extends AbstractRouter<R, M> {

    public GenericServersRouter(final Map<ProtocolType, Route<R, M>> routes) {
        super(routes);
    }

    @Override
    protected DeliveryResult sendUsingRoute(final Route<R, M> route, final R actor, final M message) {
        return route.sendToServer(actor, message);
    }
}
