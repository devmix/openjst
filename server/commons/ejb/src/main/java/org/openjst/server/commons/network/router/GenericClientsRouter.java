 ...
 */
class GenericClientsRouter<R extends Actor<?>, M extends Message<?>> extends AbstractRouter<R, M> {

    public GenericClientsRouter(final Map<ProtocolType, Route<R, M>> routes) {
        super(routes);
    }

    @Override
    protected DeliveryResult sendUsingRoute(final Route<R, M> route, final R actor, final M message) {
        return route.sendToClient(actor, message);
    }
}
